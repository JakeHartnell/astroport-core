...
function normalizePool(row: Record<string, unknown>) {
  ...
  const assets = assetInfos.map((asset) => ({ denom: normalizeAssetInfo(asset), valueUsd: null, priceUsd: null, priceJuno: null, priceStatus: "missing" }));
  const updatedAt = iso(row.updated_at) ?? new Date(0).toISOString();
  return {
    id: String(row.id ?? row.pair_address),
...
export class PostgresApiStore implements IndexerApiStore {
  constructor(private readonly db: Queryable, private readonly chainId: string, private readonly cursorId = "astroport-juno-v1") {}

  async health() {
    const cursor = await this.db.query(`SELECT last_height, updated_at FROM indexer_cursors WHERE id = $1`, [this.cursorId]);
    return { status: "ok", service: "astroport-juno-indexer", chainId: this.chainId, cursorHeight: toNumber(cursor.rows[0]?.last_height), cursorUpdatedAt: iso(cursor.rows[0]?.updated_at), dataSource: "indexer", isMock: false };
  }

  async ready() {
    await this.db.query("SELECT 1");
    const migrations = await this.db.query(`SELECT count(*)::int AS count FROM schema_migrations`);
    return { status: "ready", database: "ok", migrationsApplied: Number(migrations.rows[0]?.count ?? 0), dataSource: "indexer", isMock: false };
  }

  async stats() {
    const result = await this.db.query(`SELECT count(*)::int AS pool_count, max(updated_at) AS updated_at FROM pools WHERE chain_id = $1`, [this.chainId]);
    const row = result.rows[0] ?? {};
    return { poolCount: Number(row.pool_count ?? 0), tvlUsd: 0, tvlJuno: null, volume24hUsd: 0, volume24hJuno: null, volume7dUsd: 0, fees24hUsd: 0, fees24hJuno: null, incentivizedPools: 0, updatedAt: iso(row.updated_at) ?? new Date(0).toISOString(), dataSource: "indexer", isMock: false };
  }
  ...
  async pools(query: PaginationQuery) {
    ...
    const result = await this.db.query(
      `SELECT p.*, s.tvl_usd, s.tvl_juno
       FROM pools p
       LEFT JOIN LATERAL (
         SELECT tvl_usd, tvl_juno FROM pool_state_snapshots WHERE pool_id = p.id ORDER BY height DESC LIMIT 1
       ) s ON true
       WHERE p.chain_id = $1 AND ($2::text IS NULL OR p.pair_address = $2)
       ORDER BY COALESCE(s.tvl_usd, 0) DESC, p.created_height DESC NULLS LAST
       LIMIT $3 OFFSET $4`,
  ...
  async pool(id: string) {
    const result = await this.db.query(
      `SELECT p.*, s.tvl_usd, s.tvl_juno FROM pools p
       LEFT JOIN LATERAL (SELECT tvl_usd, tvl_juno FROM pool_state_snapshots WHERE pool_id = p.id ORDER BY height DESC LIMIT 1) s ON true
  ...
  async candles(id: string, query: PaginationQuery) {
    ...
    const result = await this.db.query(
      `SELECT pair_address, pool_id, asset, quote_asset, interval, bucket_start, open, high, low, close, volume, volume_usd, trade_count
       FROM token_candles
  ...
  async walletHistory(addr: string, query: PaginationQuery) {
    const result = await this.db.query(
      `SELECT tx_hash, provider AS wallet_address, pair_address, kind AS type, height, block_time AS timestamp, NULL::jsonb AS offer_asset, NULL::jsonb AS ask_asset, NULL::numeric AS amount_usd, NULL::numeric AS fee_usd, true AS success
       FROM liquidity_events WHERE chain_id = $1 AND provider = $2
       ORDER BY height DESC LIMIT $3 OFFSET $4`,
