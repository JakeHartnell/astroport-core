...
function intEnv(name: string, fallback: number, options: IntEnvOptions = {}): number {
  const value = process.env[name];
  if (!value) return fallback;
  const trimmed = value.trim();
  if (!/^\d+$/.test(trimmed)) throw new Error(`${name} must be ${options.label ?? "a non-negative integer"}`);
  const parsed = Number.parseInt(trimmed, 10);
  if (!Number.isSafeInteger(parsed) || parsed < (options.min ?? 0)) {
    throw new Error(`${name} must be ${options.label ?? "a non-negative integer"}`);
  }
  return parsed;
}
...
export function loadConfig(): IndexerConfig {
  ...
  const fetchWindowSize = intEnv("FETCH_WINDOW_SIZE", 250, { min: 1, label: "an integer greater than or equal to 1" });
  const fetchConcurrency = intEnv("FETCH_CONCURRENCY", 32, { min: 1, label: "an integer greater than or equal to 1" });
  if (fetchConcurrency > fetchWindowSize) {
    throw new Error("FETCH_CONCURRENCY must be less than or equal to FETCH_WINDOW_SIZE");
  }

  return {
    databaseUrl: env("DATABASE_URL", "postgres://postgres:***@localhost:5432/astroport_indexer"),
    rpcUrl,
    ...
    batchSize: intEnv("BATCH_SIZE", 20, { min: 1, label: "an integer greater than or equal to 1" }),
    dryRun: boolEnv("DRY_RUN"),
    cursorId: env("CURSOR_ID", "astroport-juno-v1"),
    indexerMode: indexerModeEnv(),
    rangeSize: intEnv("RANGE_SIZE", 5_000, { min: 1, label: "an integer greater than or equal to 1" }),
    fetchWindowSize,
    fetchConcurrency,
    realtimeFetchConcurrency: intEnv("REALTIME_FETCH_CONCURRENCY", 8, { min: 1, label: "an integer greater than or equal to 1" }),
